...
from layers.convolution import conv2d_transpose, conv2d, depthwise_separable_conv2d

import tensorflow as tf


class UNetDepthNet_w_kp_separable_conv(BasicModel_w_kp):
    ...
    def init_kp_refinement_branch(self):
        ...
        with tf.name_scope('kp_refine'):
            ...
            self.refine1 = depthwise_separable_conv2d('refine1', self.concat, width_multiplier=self.encoder.width_multiplier, padding='SAME', stride = (1,1),
                                batchnorm_enabled=batchnorm_enabled, is_training=self.is_training,
                                num_filters=32, kernel_size=(9,9), activation=tf.nn.relu6,
                                dropout_keep_prob=self.args.dropout_keep_prob,
                                l2_strength=self.encoder.wd)
            self._debug(self.refine1)
            self.refine2 = depthwise_separable_conv2d('refine2', self.refine1, width_multiplier=self.encoder.width_multiplier, padding='SAME', stride = (1,1),
                                batchnorm_enabled=batchnorm_enabled, is_training=self.is_training,
                                num_filters=32, kernel_size=(13,13), activation=tf.nn.relu6,
                                dropout_keep_prob=self.args.dropout_keep_prob,
                                l2_strength=self.encoder.wd)
            self._debug(self.refine2)
            self.refine3 = depthwise_separable_conv2d('refine3', self.refine2, width_multiplier=self.encoder.width_multiplier, padding='SAME', stride = (1,1),
                                batchnorm_enabled=batchnorm_enabled, is_training=self.is_training,
                                num_filters=64, kernel_size=(13,13), activation=tf.nn.relu6,
                                dropout_keep_prob=self.args.dropout_keep_prob,
                                l2_strength=self.encoder.wd)
            self._debug(self.refine3)
            self.refine4 = depthwise_separable_conv2d('refine4', self.refine3, width_multiplier=self.encoder.width_multiplier, padding='SAME', stride = (1,1),
                                batchnorm_enabled=batchnorm_enabled, is_training=self.is_training,
                                num_filters=128, kernel_size=(15,15), activation=tf.nn.relu6,
                                dropout_keep_prob=self.args.dropout_keep_prob,
                                l2_strength=self.encoder.wd)
            self._debug(self.refine4)
            self.refine5 = depthwise_separable_conv2d('refine5', self.refine4, width_multiplier=self.encoder.width_multiplier, padding='SAME', stride = (1,1),
                                batchnorm_enabled=batchnorm_enabled, is_training=self.is_training,
                                num_filters=256, kernel_size=(1,1), activation=tf.nn.relu6,
                                dropout_keep_prob=self.args.dropout_keep_prob,
                                l2_strength=self.encoder.wd)
            self._debug(self.refine5)
            self.refine6 = depthwise_separable_conv2d('refine6', self.refine5, width_multiplier=self.encoder.width_multiplier, padding='SAME', stride = (1,1),
                                batchnorm_enabled=batchnorm_enabled, is_training=self.is_training,
                                num_filters=16, kernel_size=(1,1), activation=tf.nn.relu6,
                                dropout_keep_prob=self.args.dropout_keep_prob,
                                l2_strength=self.encoder.wd)
            self._debug(self.refine6)
            self.refine7 = depthwise_separable_conv2d('refine7', self.refine6, width_multiplier=self.encoder.width_multiplier, padding='SAME', stride = (1,1),
                                batchnorm_enabled=batchnorm_enabled, is_training=self.is_training,
